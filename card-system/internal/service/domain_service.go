...
func (s *DomainService) AllocateDomain(merchantID uint) (string, error) {
	...
	domain := &models.Domain{
		...
		CertStatus:  models.CertStatusApplied,
	...
	go func() {
		for range ticker.C {
			...
			utils.DB.Where("cert_status = ? AND cert_expire < ?", 
				models.CertStatusApplied, time.Now().Add(7*24*time.Hour)).Find(&domains)
...
func (d *Domain) ApplyCertificate() error {
	...
	if err != nil {
		d.CertStatus = models.CertStatusFailed
	...
	d.CertStatus = models.CertStatusApplied
